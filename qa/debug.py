...
def main():
    ...
    # Check DOM structure
    print("\n=== DOM Structure ===")
    html = ws_eval(ws, "document.documentElement.outerHTML.slice(0,5000)", msg_id=15)
    print(html[:3000])
    
    print("\n=== Key elements ===")
    for i, sel in enumerate(['#productGrid', '#catTabs', '.cat-tab', '.card', '#langBtn', 'header']):
        count = ws_eval(ws, f"document.querySelectorAll('{sel}').length", msg_id=16+i)
    ...
    if has_dd:
        ...
        if dd:
            cats_count = len(dd.get('categories',[]))
            print(f"  defaultData().categories: {cats_count}")
